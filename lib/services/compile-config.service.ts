...
export function buildCompileConfig(
  ...
  return merge(
    getJsConfig(cfg, bundle),
    getCssConfig(cfg, bundle),
    getNoScriptConfig(bundle),
  );
}

function getJsConfig(cfg: WordPackConfig, bundle: BundleConfig): Configuration {
          ...
          use: {
            ...
            options: {
              env: {
                mode: 'usage',
                coreJs: '3.38',
                targets: resolveBrowserTargets(cfg),
              },
              jsc: {
                parser: {
                  syntax: 'typescript',
...
function resolveBrowserTargets(cfg: WordPackConfig): string[] {
  ...
  return {
    ...
    plugins: [
      new MiniCssExtractPlugin({
        filename: ({ chunk }: PathData) => cssName(chunk?.name, bundle, cfg),
      }),
...
function getNoScriptConfig(bundle: BundleConfig): Configuration {
